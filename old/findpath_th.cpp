										...
										int width, int maxE)
{
	...
	do
	{
		...
		// dont want to deal with mutexes, we just append to two vectors and then join them


        std::future<int> ret1 = std::async(&find_path_once, vc, pt1, pt2, maxl, maxE, width);
        path_fwd = !path_fwd;
        std::future<int> ret2 = std::async(&find_path_once, vc, pt2, pt1, maxl, maxE, width);          
