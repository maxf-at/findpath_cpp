										...
										int width, int maxE)
{
	...
	do
	{
		...
		// fix this: result here wont get printered
		// path_fwd = !path_fwd;
		...
		for (d = 0; d <= BP_dist; d++)
            {
				if (route[d].move.pos_5==0 and route[d].move.pos_3==0)
					continue;
                // printf("(%d/%d/%d/%6.0f); ", d+1, route[d].move.pos_5, route[d].move.pos_3, route[d].en*100);
...
// std::unordered_map<std::int16_t*, int*> loops;
// std::unordered_map<std::string, std::string> loops;

// std::unordered_map< array<int,80> , array<int,80> > loops;

struct ArrayHasher {
    std::size_t operator()(const std::array<int, 80>& a) const {
        std::size_t h = 0;
...
string convertToString16(std::int16_t* a, int size) 
{ 
    int i; 
    string s = ""; 
    for (i = 0; i < size; i++) { 
					  ...
					  intermediate_t *next, int dist)
{
    ...
    std::array<short,80> Y;
    std::copy(&c.pt, &c.pt+80, std::begin(Y));


    // if (loops.contains(c.pt))
    // {        
    //     loopidx = loops[c.pt];
    //     printf("exists\n");
    // }
