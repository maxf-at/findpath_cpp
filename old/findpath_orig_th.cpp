...
PRIVATE int find_path_once(vrna_fold_compound_t* vc, short* pt1, short* pt2, int maxl, int maxE);
...
PUBLIC int vrna_path_findpath_saddle(vrna_fold_compound_t* vc, const char* s1, const char* s2, int width)
{
    return vrna_path_findpath_saddle_ub(vc, s1, s2, width, INT_MAX - 1);
}

PUBLIC int vrna_path_findpath_saddle_ub(vrna_fold_compound_t* vc, const char* s1, const char* s2, int width, int maxE)
{
    int     maxl;
    short * ptr, *pt1, *pt2;
    ...
    move_t* fwd_path = nullptr;
    move_t* bwd_path = nullptr;

    //   maxl = 1;
    maxl = 16;

    //   int multiplier = (width-20.0)/3.0;
    constexpr int multiplier = 4;

    // do {
    //     int saddleE;
    //     path_fwd = !path_fwd;
    //     if (maxl > width) maxl = width;

    //     if (path) free(path);

    //     // printf ("\nmaxl:%d,width:%d,multiplier:%d\n", maxl, width, multiplier);

    //     // if (maxl*(multiplier+multiplier*0.1) > width){
    //     //     // printf ("last\n");
    //     //     maxl = width;
    //     // }
    ...
    //     path_fwd              = !path_fwd;
    //     std::future<int> ret1 = std::async(&find_path_once, vc, pt1, pt2, maxl, maxE);
    //     path_fwd              = !path_fwd;
    //     std::future<int> ret2 = std::async(&find_path_once, vc, pt2, pt1, maxl, maxE);
    //     // // saddleE = std::min(ret1.get(),ret2.get());
    ...
    do {
        ...
        path_fwd              = !path_fwd;
        std::future<int> ret1 = std::async(&find_path_once, vc, pt1, pt2, maxl, maxE_fwd);
        // saddle_fwd = find_path_once(vc, pt1, pt2, maxl, maxE_fwd);
        path_fwd              = !path_fwd;

        // std::future<int> ret2 = std::async(&find_path_once, vc, pt2, pt1, maxl, maxE_bwd);

        // saddle_bwd = find_path_once(vc, pt1, pt2, maxl, maxE_bwd);
        // saddleE = std::min(ret1.get(),ret2.get());
    ...
    printf ("\nmaxe:%d,fwd:%d,bwd:%d\n", maxE, maxE_fwd, maxE_bwd);

    return maxE;
}

PUBLIC vrna_path_t* vrna_path_findpath(vrna_fold_compound_t* fc, const char* s1, const char* s2, int width)
{
...
PUBLIC vrna_path_t* vrna_path_findpath_ub(vrna_fold_compound_t* fc, const char* s1, const char* s2, int width, int maxE)
{
    struct vrna_path_options_s* opt;
    vrna_path_t*                path;
...
{
    struct vrna_path_options_s* options = (struct vrna_path_options_s*)vrna_alloc(sizeof(struct vrna_path_options_s));
...
PRIVATE vrna_path_t* findpath_method(vrna_fold_compound_t* fc, const char* s1, const char* s2, int width, int maxE,
...
PUBLIC vrna_path_t* vrna_path_direct_ub(vrna_fold_compound_t* fc, const char* s1, const char* s2, int maxE,
                                        struct vrna_path_options_s* options)
{
    int                         E, d;
...
PRIVATE int find_path_once(vrna_fold_compound_t* vc, short* pt1, short* pt2, int maxl, int maxE)
{
    ...
    for (d = 1; d <= dist; d++) {
        ...
        for (c = 0; current[c].pt != NULL; c++) num_next += try_moves(vc, current[c], maxE, next + num_next, d);
        if (num_next == 0) {
        ...
        /* free the old stuff */
        for (cc = current; cc->pt != NULL; cc++) free_intermediate(cc);
        for (u = 0; u < maxl && u < num_next; u++) current[u] = next[u];
