...
typedef struct intermediate
{
	...
	bool operator<(const intermediate& a) const{

		const auto c = memcmp(a.pt, this->pt, a.pt[0] * sizeof(short));
		if (c != 0){
...
auto compare_ptables_n(const intermediate_t &a, const intermediate_t &b) -> bool
{
	...
	const auto c = memcmp(a.pt, b.pt, a.pt[0] * sizeof(short));


	// fprintf(stderr, "compare c=%d, %d %d", c, a.pt[0], b.pt[0]);
	// std::string test;
	// std::cin >> test;
	...
	const auto e_diff = a.curr_en - b.curr_en;
	if (e_diff > 0)
			return true;
		else
			return false;	
					  ...
					  intermediate_t *next, int dist)
{
	...
	{
		int i, j;
		if (mv->when > 0) // move not available
			continue;
...
#endif
		if (en < maxE) // maxE is from perv. iteration!
		{
						   ...
						   int maxl, int maxE)
{
	...
	// this generates the move list, with when=0 throughout.
	for (i = 1; i <= len; i++)
	{
		if (pt[i] != pt2[i])
		{
	...
	BP_dist = dist;
	current = (intermediate_t *)vrna_alloc(sizeof(intermediate_t) * (maxl + 1)); // 1000
	current[0].pt = pt;
	...
	// std::vector<intermediate_t> next_2(dist * maxl + 1);
	// next_2.resize(dist * maxl + 1);
	// intermediate_t* next = next_2.data();

 	// std::array<intermediate_t,500000> next_2;
	// intermediate_t* next = next_2.begin();
	...
	for (d = 1; d <= dist; d++)
	{
		...
		// iterate over last unique ptables
		...
		// std::stable_sort(next_2.begin(), next_2.begin()+num_next, compare_ptables_n);

		std::sort(next, next+num_next);
		// parasort(num_next, next, 6);
		
		// std::stable_sort(next_2.begin(), next_2.begin()+num_next,
		// 	[](const intermediate_t &a, const intermediate_t &b) -> bool
		// 	{   // sorting lambda
		// 		const auto c = memcmp(a.pt, b.pt, a.pt[0] * sizeof(short));
		// 		if (c != 0){
		// 			if (c>0)
		...
		// std::stable_sort(next_2.begin(), next_2.begin()+num_next, compare_energy_n);
		...
		// print_moves(next,num_next,u);

		/* free the old stuff */
		// in the last step all remaining moves will be deleted here!
		// (since there is only 1 final ptable)

		for (cc = current; cc->pt != NULL; cc++){

			int offset = next-cc;
			int current_opt = cc->Sen;
			print_moves(cc,1,6);
			IC(current_opt);
			if (d != 6 || current_opt >-70){
				fprintf(stderr, "erase cc = %d, bp_dist = %d ", offset, d);	
				free_intermediate(cc);
			}
			else
			{
				fprintf(stderr, "dont erase cc = %d, bp_dist = %d ", offset, d);	
			}
		...
		for (u = 0; u < maxl && u < num_next; u++){
			// copy struct ?
			// IC(u, "current=next");
			print_moves(next+u,1,5);
			fprintf(stderr, "current %d = next \n", u);	
			// sorted lookahead gets copied into current paths
			current[u] = next[u];
			}
		for (; u < num_next; u++){
			fprintf(stderr, "erase u = %d\n", u);	
	...
	IC(w_time/1000000);
	IC(current);
	// print_moves(current,1,1);
	// ic.disable();
	
	path = current[1].moves;

	// iterate over all current paths
	for (intermediate_t *cc = current; cc->pt != NULL; cc++)
	{
		// it[l].moves[k].when
		// iterate over BP_dist to find 0 entry for when
		for (d = 0; d < dist; d++)
		{
			if (cc->moves[d].when == 0)
				cc->moves[d].when = 6;
		}
