...
auto main(int argc, char** argv) -> int
{
    ...
    short* pt_1 = vrna_ptable_copy(G1.pt_1);
    short* pt_2 = vrna_ptable_copy(G1.pt_2);

    for (int i = 1; i < pt_1[0]; i++) {
        // fmt::print("i {}\n", i);
        if ((pt_1[i] == 0 and G2.pt_1 != 0) or (pt_1[i] != 0 and G2.pt_1 == 0)) {
            pt_1[i] = G2.pt_1[i];
        }
        if ((pt_2[i] == 0 and G2.pt_2 != 0) or (pt_2[i] != 0 and G2.pt_2 == 0)) {
            pt_2[i] = G2.pt_2[i];
        }
    }

    int current_en_1 = vrna_eval_structure_pt(fc, pt_1);
    int current_en_2 = vrna_eval_structure_pt(fc, pt_2);

    fmt::print("{} {} {}\n", vrna_db_from_ptable(pt_1), G1.bp_dist, current_en_1);
    fmt::print("{} {} {}\n", vrna_db_from_ptable(pt_2), G2.bp_dist, current_en_2);

    int total_bp_dist = G1.bp_dist + G2.bp_dist;

    // int max_en             = INT_MAX;
    int max_en = -1290;
    int merge_search_width = 60;
    ...
    current_path.current_en      = current_en_1;
    current_path.current_s       = current_en_1;
    ...
    for (int d = 1; d <= total_bp_dist; d++) {
        ...
        for (const auto current_path : all_paths) {
            ...
            for (const auto& current_edge : G1_node.out_edges) {
                ...
                auto        current_en = current_path.current_en +
                                  vrna_eval_move_pt(fc, current_path.current_ptable, i, j);
