...
{
	std::int16_t *pt;	   /**<  @brief  pair table */
    std::int16_t dist;	   /**<  @brief  pair table */
	std::int16_t i; /* i,j>0 insert; i,j<0 delete */
	std::int16_t j;
	std::int32_t curr_en;   /**<  @brief  current energy */
	

    bool operator == (graph_element const &b) const {
        return !memcmp(&b, this, sizeof(graph_element) );
          }
    bool operator < (graph_element const &b) const {
        // printf("compare %d %d %d", &b, this, memcmp(&b.pt, this->pt, sizeof(graph_element)));
        // auto c = memcmp(b.pt, this->pt, b.pt[0] * sizeof(short));
        auto c = memcmp(&b, this, sizeof(graph_element));
    ...
    // {
    //     // write obj to stream
    //     return os << this->curr_en;
    // }


} graph_element_t;

std::ostream& operator<<(std::ostream& os, graph_element b)
{
    return os << b.curr_en;
}
...
std::vector<move_t*> global_results;
...
auto main(int argc, char *argv[]) -> int
{
	...
	printf("Seq input\n");
	// std::cin >> l;
    l = "AUCAAGCUUUUGUAGCUAAACCUACGCGGGUUUGGCGUAGGGGGU";
	...
	vrna_path_gradient(fc, pt3, VRNA_PATH_NO_TRANSITION_OUTPUT);



    digraph<graph_element_t> G;
    graph_element_t node1 {pt1, 0, 0, 0, 5};
    graph_element_t node2 {pt2, 2, 0, 0, 6};
    graph_element_t node3 {pt2, 2, 0, 0, 9};

    std::cout << node1.curr_en << "\n";
    std::cout << node2.curr_en << "\n";
    ...
    if (node1==node2)
        std::cout << "n1 n2 identical \n";
    else
        std::cout << "not n1 n2 identical \n";
    if (node1<node2)
        std::cout << "< true \n";
    else
        std::cout << "< false \n";    
    ...
    G.add(node1, node2, -22, -11);
    G.add(node1, node3, 3, 3);
    G.add(node2, node1, -25, 41);

    // G.add(node1, node2, -22);
    // G.add(node2, node1, -22);
    // G.add(node2, node1, -25);
    ...
    IC(global_results);
    for (auto &c : global_results) {
        
        // qsort(c, BP_dist, sizeof(move_t), compare_moves_when);
        for (int d = 0; d < BP_dist; d++)
        {   
            printf("(%d/ %d / %d / %d); ", c[d].when, c[d].i, c[d].j, c[d].E);
        }

        printf("\n");
        // print_moves_o(c, 10, 1);
    ...
    //     std::cout << n.curr_en << "node\n";
    //     for (const auto& c : G.connections(n)) {
    //         std::cout << c.first.curr_en << "/" << c.second[0] << c.second[1] << " / connection with node\n";
    //     }
    // }
					  ...
					  intermediate_t *next, int dist)
{
	...
	for (mv = c.moves; mv->i != 0; mv++)
	{
		...
		// if (en == maxE && next[num_next].moves != nullptr){
        // // if (en == maxE){
		// 	// IC(next[num_next].moves, maxE, dist);
        //     // print_moves(next+num_next,1,dist+1);

        //     move_t *temp_moves = copy_moves(next[num_next].moves);
        //     qsort(temp_moves, BP_dist, sizeof(move_t), compare_moves_when);

        //     // for (int k; k<sizeof(dist+1); k++)
            
        //     for (int k=0; k<dist; k++)
        //     {
        //         // IC(k);
        //         auto temp_i = (temp_moves+k)->i ;
        //         auto temp_j = (temp_moves+k)->j ;
        //         const auto temp_en = (temp_moves+k)->E ;
        //         auto temp_when = (temp_moves+k)->when ;
        //         if (temp_when==0)
        //             continue;
        ...
        //             temp_i = -temp_i;
        //             temp_j = -temp_j;
        //             temp_when = dist-temp_when+2;

        //         }
        //         IC(k, dist, temp_when, temp_i, temp_j, temp_en);
        //         printf("(%d/%d/%d/%d); ", temp_when, temp_i, temp_j, temp_en);
        ...
        //     // print_moves(temp_moves,1,dist+1);

        //     // IC(next[num_next].move_en);
		...
		if (en <= maxE)
		{
			...
			// const auto Sen = next[num_next].Sen;
			// IC(next[num_next].move_en, min_before, min_after, Sen, opt);
						   ...
						   int maxl, int maxE, int search_width)
{
		...
		{		
			...
			if (memcmp(next[u].pt, next[c].pt, sizeof(short) * len) != 0)
			{
                ...
                if (d>=dist && next[c].Sen <= maxE && maxl>=search_width){
                    ...
                    {
                        qsort(temp_moves, BP_dist, sizeof(move_t), compare_moves_when_rev);
                    }
                    // IC("D", path_fwd, maxl, search_width, maxE,  next[c].Sen);
                    global_results.push_back(temp_moves);
    ...
    // move_t *temp_moves = copy_moves(current[0].moves);
    // if (path_fwd) {
    //     qsort(temp_moves, BP_dist, sizeof(move_t), compare_moves_when);
    // }
    // else
