...
auto compare_sort(const intermediate_t &a, const intermediate_t &b) -> int
{
						   ...
						   int maxl, int maxE)
{
	...
	fprintf(stderr, "fp_once function");
	...
	for (d = 1; d <= dist; d++)
	{
		...
		// for (c = 0; c >= 10; c++)
			...
			fprintf(stderr, "bp_dist = %d / c =  %d / %d / call try_moves\n", d, c, num_next);
			...
			fprintf(stderr, " %d %s %6.2f", c, temp_str, test_E);				
			fprintf(stderr, "\n");
		...
		if (num_next == 0)
		{
			fprintf(stderr, "break, num_next = %d  \n", num_next);
		...
		fprintf(stderr, "received candidates: num_next = %d  \n", num_next);
		...
		std::sort(next, next+num_next, compare_sort);
		...
		{		
			...
			if (c == 0) {
				//fprintf(stderr, "c=0%s %6.2f", temp_str, test_E);				
				//fprintf(stderr, "\n");
			...
			if (memcmp(next[u].pt, next[c].pt, sizeof(short) * len) != 0)
			{
				...
				// float test_E = vrna_eval_structure(vc, temp_str);
				fprintf(stderr, " A %s %6.2f", temp_str, test_E);				
				fprintf(stderr, "\n");
				...
				fprintf(stderr, " D %s %6.2f duplicate", temp_str, test_E);				
				fprintf(stderr, "\n");
		...
		for (u = 0, c = 0; c < num_next; c++)
		{
			...
			// float test_E = vrna_eval_structure(vc, temp_str);
			fprintf(stderr, " S %s %6.2f", temp_str, test_E);				
			fprintf(stderr, "\n");
		}
		/* free the old stuff */
		for (cc = current; cc->pt != NULL; cc++)
