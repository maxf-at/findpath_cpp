...
typedef struct intermediate
{
    ...
    std::int32_t min_en_a;   
    std::int32_t min_en_b;   
	std::int16_t min_pos;
    std::int32_t min_en_post; 
...
int
vrna_path_findpath_saddle_2(vrna_fold_compound_t *fc,
                             const char           *s1,
                             const char           *s2,
                             int                  width,
                             int                  maxE,
...
PRIVATE int compare_ptable_simple(const void *A, const void *B);
PRIVATE int compare_energy_simple(const void *A, const void *B);
...
PRIVATE int try_moves(vrna_fold_compound_t *vc, intermediate_t c, int maxE,
					  intermediate_t *next, int dist, int e1_en);
...
{
	return vrna_path_findpath_saddle_2(vc, s1, s2, width, INT_MAX - 1, path, path);
}

PUBLIC int vrna_path_findpath_saddle_2(vrna_fold_compound_t *vc,
										const char *s1, const char *s2,
										int width, int maxE, move_t *fwd_path, move_t *bwd_path)
{
	int maxl;
	short *ptr, *pt1, *pt2;
	...
	do
	{
        ...
        // maxE = 360;
		saddle_fwd = find_path_once(vc, pt1, pt2, maxl, maxE_fwd, width);
		IC("fwd", saddle_fwd, maxl, maxE_fwd);
		// path_fwd = !path_fwd;

		if (saddle_fwd <= maxE_fwd)
		{
			maxE_fwd = saddle_fwd;
			// if (bestpath)
			// 	free(bestpath);
        ...
        // comment below to disable bwd pass

		// saddle_bwd = find_path_once(vc, pt2, pt1, maxl, maxE_bwd, width);
		// // IC("bwd", saddle_bwd, maxl, maxE_bwd);
        // if (saddle_bwd <= maxE_bwd)
		// {
		// 	maxE_bwd = saddle_bwd;
		// 	// if (bestpath)
		// 	// 	free(bestpath);
		...
		// 	path = NULL;
		// }
		// saddleE = std::max(saddle_fwd, saddle_bwd);
    ...
    maxE = std::min(maxE_fwd, maxE_bwd);
...
//   } while (maxl < 2 * width);
    ...
    if (fwd_path) {   
         path_fwd = 1;
        qsort(fwd_path, BP_dist, sizeof(move_t), compare_moves_when);
        print_moves_o(fwd_path, BP_dist, BP_dist);
    }
    if (bwd_path) {   
        path_fwd = 0;
        qsort(bwd_path, BP_dist, sizeof(move_t), compare_moves_when);
        print_moves_o(bwd_path, BP_dist, BP_dist);
    }
									 ...
									 unsigned int return_type)
{
    ...
    move_t *fwd_path, *bwd_path = nullptr;

	E = vrna_path_findpath_saddle_2(fc, s1, s2, width, maxE, fwd_path, bwd_path);
	...
	if (E < maxE)
	{
		...
		// for (d = 0; d <= BP_dist; d++)
        //     {
		// 		if (route[d].move.pos_5==0 and route[d].move.pos_3==0)
		// 			continue;
					  ...
					  intermediate_t *next, int dist, int e1_en)
{
	...
	for (mv = c.moves; mv->i != 0; mv++)
	{
		...
		if (en <= maxE+c.min_en_a)
		// if (en < maxE)
		{
			// next[num_next].move_en = c.move_en; // copy moves
			// next[num_next].move_en.resize(dist+1);
			// // next[num_next].move_en.push_back(en);			
			// next[num_next].move_en[dist-1] = en;		
			
			// if (en>=oldE) {
			// 	next[num_next].min_pos = dist;
			// }

			// const auto min_pos = next[num_next].min_pos;
			// const auto vec_elems = next[num_next].move_en.size();
			// const auto begin_pos = std::begin(next[num_next].move_en);
			// const auto saddle_pos = std::begin(next[num_next].move_en) + min_pos;
			// const auto end_pos = std::end(next[num_next].move_en);

			// int min_before = *std::min_element(begin_pos, saddle_pos); 
			// int min_after = *std::min_element(saddle_pos-1, end_pos); 

			// // def. wrong
			// // min_before *= min_pos;
			// // min_after *= (vec_elems-min_pos+1);
			// // min_before *= (vec_elems-min_pos+1);
			// // min_after *= min_pos;
			// const auto opt = min_before+min_after;

			// // int min_before = *std::min_element(std::begin(next[num_next].move_en),
			// // 		std::end(next[num_next].move_en) ); 
			// next[num_next].opt = opt;

			// oldE is current[0].Sen
			// next[num_next].Sen = (en > oldE) ? en : oldE;



            // set saddle energy & its position
            if (en>oldE){

 

                next[num_next].Sen = en;
                next[num_next].max_pos = dist;

                c.min_en_a = std::min(c.min_en_a, c.min_en_post);

 
                // next[num_next].min_en_a = std::min(c.min_en_b, c.min_en_a);
                // next[num_next].min_en_b = next[num_next].Sen;
                
            }
            else // current energy isn't a saddle
            {
                next[num_next].Sen = oldE;
                next[num_next].max_pos = c.max_pos;


                // next[num_next].min_en_b = (en < c.min_en_b) ? en : c.min_en_b;
            }

            auto const en_difference =  -e1_en +en; 

            // next[num_next].min_en_b = c.min_en_a;
            // if (en < c.min_en_a) {
            //     next[num_next].min_en_a = en;
            // }
            // else
            // {
            //     next[num_next].min_en_a = c.min_en_a; // get min from before
            // }       

            // use minpos, use when sorting...

            next[num_next].min_en_b = c.min_en_a;
            // if (en_difference < 0) {
            // if (en_difference < c.min_en_a) {
            if (en_difference < c.min_en_a && dist<next[num_next].max_pos) {


                next[num_next].min_pos = dist;
                next[num_next].min_en_a = en_difference;
            }
            else
            {
                // post saddle
                if (en_difference < c.min_en_a and en_difference > -140) {
                    next[num_next].min_en_post = en_difference;
                }
                if (en_difference == -200)
                    IC("here", c.min_en_a, c.max_pos);

                next[num_next].min_en_a = c.min_en_a; // get min from before
                next[num_next].min_pos = c.min_pos;
            }     
            

            // if (en < e1_en) {
            //     next[num_next].min_en_a = c.min_en_a + (-e1_en +en);
            // }
            // else
            // {
            //     next[num_next].min_en_a = c.min_en_a; // get min from before
            ...
            // if (next[num_next].Sen < -130) // last run...
			// 	next[num_next].Sen = -130;
			// next[num_next].Sen = -130;
			...
			IC(dist, i, j, next[num_next].Sen, en);
            IC(c.min_en_a, c.min_en_b, en, next[num_next].min_en_a, next[num_next].min_en_b, next[num_next].min_en_post, next[num_next].max_pos);
           
            // IC(c.min_en_a, next[num_next].min_en_a, next[num_next].max_pos);
						   ...
						   int maxl, int maxE, int search_width)
{
	...
	// IC(maxE, maxl, search_width, path_fwd);
    ...
    current[0].min_en_a = 0;
    // current[0].min_en_a = INT_MAX;
    // current[0].min_en_b = INT_MAX;


    int e1_en = current[0].Sen;
	...
	for (d = 1; d <= dist; d++)
	{
		...
		for (c = 0; current[c].pt != NULL; c++){
			...
			num_next += try_moves(vc, current[c], maxE, next + num_next, d, e1_en);
        ...
        if (maxl==search_width) // last iteration
		    qsort(next, num_next, sizeof(intermediate_t), compare_ptable_simple);
		...
		{		
			...
			if (memcmp(next[u].pt, next[c].pt, sizeof(short) * len) != 0)
			{
                ...
                // if (d>=dist && next[c].Sen <= maxE && maxl*2>=search_width){
				// if (next[c].Sen <= maxE && maxl>=search_width){
        ...
        if (maxl==search_width) // last iteration
		    // qsort(next, num_next, sizeof(intermediate_t), compare_energy_simple);
	...
	// IC(maxl, search_width);
	// if (path and maxl*2>=search_width){
	if (path){
		move_t *temp_moves = copy_moves(path);
		qsort(temp_moves, BP_dist, sizeof(move_t), compare_moves_when);
		print_moves_o(temp_moves, dist, d); // prints also greedy results
	}
...
PRIVATE int compare_ptable(const void *A, const void *B)
{
    ...
    // auto const opt_constraint = a->Sen+a->min_en_a*100+a->min_en_b*100 - b->Sen-b->min_en_a*100-b->min_en_b*100;
    // auto const opt_constraint = a->Sen+a->min_en_a*100 - b->Sen-b->min_en_a*100;
    auto opt_constraint = a->min_en_a - b->min_en_a;
    // if (a->min_en_a == b->min_en_a)
    if (opt_constraint == 0)
        opt_constraint = a->min_en_b - b->min_en_b;
        // auto const opt_constraint = a->Sen+a->min_en_b*100 - b->Sen-b->min_en_b*100;
    // if (opt_constraint == 0)
    if (opt_constraint == 0 and a->min_en_a!= 0)    
        opt_constraint = a->Sen - b->Sen;
    // opt_constraint = a->Sen - b->Sen;

    // ignore if we're on the wrong side of the saddle
    // min below e1 has to be below 0 on both instances.

    if ((a->min_pos > a->max_pos) && (b->min_pos <= b->max_pos) && (a->min_en_a != 0)  && (b->min_en_a != 0) ){
        // printf("ignore a %d %d / %d %d\n", a->min_pos, a->max_pos, a->min_en_a, b->min_en_a);
        // printf("ignore sen %d %d\n",  a->Sen, b->Sen);
        return 1;
    }
    if ((a->min_pos < a->max_pos) && (b->min_pos >= b->max_pos) && (a->min_en_a != 0)  && (b->min_en_a != 0) ){
        // printf("ignore n %d %d / %d %d\n", a->min_pos, a->max_pos, a->min_en_a, b->min_en_a);
        // printf("ignore sen %d %d\n",  a->Sen, b->Sen);
        return -1;
    }

	if (opt_constraint != 0){
	    // IC("opt p", opt_constraint,  a->min_en_a, b->min_en_a, a->min_en_b, b->min_en_b);
		// printf("ignore sen %d %d\n",  a->Sen, b->Sen);
		return opt_constraint;		
		}

    // IC("c", a->Sen, b->Sen, a->curr_en, b->curr_en);
	return a->curr_en - b->curr_en;
}
...
PRIVATE int compare_energy(const void *A, const void *B)
{
	...
	if (opt_constraint != 0){
	    // IC("opt e", opt_constraint,  a->min_en_a, b->min_en_a, a->min_en_b, b->min_en_b);
		// printf("ignore sen %d %d\n",  a->Sen, b->Sen); 
        return opt_constraint;		
		}
