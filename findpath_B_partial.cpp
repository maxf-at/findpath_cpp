...
struct im {
    // short*  pt;      /**<  @brief  pair table */
    std::array<short, 301> pt;
    int     Sen;     /**<  @brief  saddle energy so far */
    int     curr_en; /**<  @brief  current energy */
    std::array<move_t, 95> moves;
    // move_t* moves;   /**<  @brief  remaining moves to target */
};
...
auto find_path_once(vrna_fold_compound_t* vc, short* pt1, short* pt2, int current_search_width, int maxE,
                    bool direction, int search_width) -> std::vector<sorted_path>;
                                     ...
                                     int width)
{
    ...
    obj = custom_findpath_method(vc, pt1, pt2, width, INT_MAX - 1);
...
// ub = upper bound?
{
    ...
    do {
        ...
        std::future<std::vector<sorted_path>> ret1 = std::async(std::launch::async, &find_path_once, vc, pt1, pt2, current_search_width, maxE, direction, width);
        // std::future<std::vector<sorted_path>> ret2 = std::async(std::launch::async, &find_path_once, vc, pt1, pt2, current_search_width, maxE, not direction, width);
...
/*
 ...
 #################################
 */
PRIVATE int try_moves(vrna_fold_compound_t* vc, im c, int maxE, im* next,
                      int dist, int bp_dist)
{
    ...
    len     = c.pt[0];
    loopidx = vrna_loopidx_from_ptable(c.pt.begin());
    oldE    = c.Sen;
    for (mv = c.moves.begin(); mv->i != 0; mv++) {
        int i, j;
...
#ifdef LOOP_EN
        en = c.curr_en + vrna_eval_move_pt(vc, c.pt.begin(), i, j);
#else
        ...
        if (en <= maxE) {
            // pt = (short*)vrna_alloc(sizeof(short) * (len + 1));
            // memcpy(pt, c.pt.begin(), (len + 1) * sizeof(short));

            std::copy(c.pt.begin(), c.pt.begin() + c.pt[0]+1, pt.begin());
            ...
            std::copy(c.moves.begin(), c.moves.begin() + bp_dist, next[num_next].moves.begin());

            // next[num_next].moves = copy_moves(c.moves.begin(), bp_dist);
            mv->when               = 0;
            num_next++;
        }
        // else {
...
auto find_path_once(vrna_fold_compound_t* vc, short* pt1_old, short* pt2_old, int current_search_width, int maxE,
                    bool direction, int final_search_width) -> std::vector<sorted_path>
{
    
    
    // std::vector<short> vec(pt1, pt1 + pt1[0]+1);
    
    std::array<short, 301> pt1;
    std::copy(pt1_old, pt1_old + pt1_old[0]+1, pt1.begin());
    
    std::array<short, 301> pt2;
    std::copy(pt2_old, pt2_old + pt2_old[0]+1, pt2.begin());

    std::array<short, 301> pt;
    std::copy(pt1_old, pt1_old + pt1_old[0]+1, pt.begin());
    ...
    im *current, *next; // array of current, array of next


    len = (int)pt1[0];
    // pt  = vrna_ptable_copy(pt1);

    std::array<move_t, 95> mlist;
    ...
    current        = (im*)vrna_alloc(sizeof(im) * (current_search_width + 1));
    current[0].pt  = pt;
    current[0].Sen = current[0].curr_en = vrna_eval_structure_pt(vc, pt1_old);
    current[0].moves                    = mlist;
    next = (im*)vrna_alloc(sizeof(im) * (dist * current_search_width + 1));
    ...
    for (d = 1; d <= dist; d++) {
        ...
        for (c = 0; current[c].pt.begin() != NULL; c++)
        // for (c = 0; current[c].pt[0] != 300; c++)
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            if (memcmp(next[u].pt.begin(), next[c].pt.begin(), sizeof(short) * len) != 0) {
                next[++u] = next[c];
                ...
                if (d >= dist && next[c].Sen <= maxE && current_search_width >= final_search_width) {
                    // if (next[c].Sen <= maxE && current_search_width>=search_width){
                    move_t* temp_moves = copy_moves(next[c].moves.begin(), bp_dist);
                    qsort(temp_moves, bp_dist, sizeof(move_t), compare_moves_when);
                    // IC("D", path_fwd, current_search_width, search_width, maxE, next[c].Sen);
    ...
    if (path) {
        ...
        for (d = 0; d < bp_dist; d++) {
            ...
            // sorted_move current_move {i,j,0};

            if (direction) {
                // fwd path
                current_path.moves[d] = {i, j, 0};
                // current_path.moves[d].i = i;
                // current_path.moves[d].j = j;

                // current_path.moves.push_back({i,j,0});
            } else {
                // bwd path, fill vector back to front
                current_path.moves[dist - d - 1] = {-i, -j, 0};
                // current_path.moves.insert(current_path.moves.begin(), {-i,-j,0});
            }
...
template<typename T>
bool isEqual(std::array<T,301> const &v1, std::array<T,301> const &v2)
{
