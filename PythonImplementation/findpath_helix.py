        ...
        while (current_bp != current_bp_end+2*len(self.moves_add)):
            ...
            for current_path in paths:
                ...
                for i, j in self.find_moves(current_p_table, end_p_table):
                    ...
                    # if limit and best_per_cluster_id[next_cluster] != i:
                    #     continue
                    ...
                    if limit and i<0:
                        ...
                        next_clusters[next_cluster].remove((i,j))

                        if (i,j) != a and (i,j) != b:
                            # delete: only extend first and last move
                            # print (i,j, a, b, (i,j) != a, (i,j) != b )
                            continue
                    ...
                    if limit and i>0:
                        ...
                        if next_clusters[next_cluster] != [] and False:
                            # extend an existing cluster
                            # there is already at least one past move in the cluster
                            # allow either a move left or right
                            left = next_clusters[next_cluster][0]
                            right = next_clusters[next_cluster][-1]
                            lpos = add_clusters[next_cluster].index(left)-1
                            rpos = add_clusters[next_cluster].index(right)+1
                            # print (left, lpos, add_clusters[next_cluster])

                            if lpos>-1: # position left of current pos is available
                                valid.append(add_clusters[next_cluster][lpos])

                            if rpos<len(add_clusters[next_cluster]):
                                valid.append(add_clusters[next_cluster][rpos])

                        # print(next_cluster, next_clusters[next_cluster], add_clusters[next_cluster], left, mid ,right)
                            
                            if (i,j) not in valid:
                                # print ("bp:", current_bp, (i,j), next_clusters[next_cluster])#, add_clusters[next_cluster],"valid", valid)
                                # print ("valid", lpos, rpos, valid, "/", add_clusters[next_cluster])
                                # print ((i,j), "invalid")
                                # print ("--")
                                continue
                        ...
                        if i>0: # greedy addition only
                            ...
                            if best_per_cluster_id[next_cluster] != i:
                                continue
                                # print ("err", i, best_per_cluster_id[next_cluster] )
                        ...
                        next_clusters[next_cluster].append((i,j))

                        # print ("bp:", current_bp, "c:", next_cluster, next_clusters[next_cluster])
                        # print ("bp:", current_bp, next_clusters)
            ...
            if limit == 2:
                ...
                for i in range(len(rest)):
                    ...
                    o = np.array([rest[i][-1].opt])
                    d = np.min(cdist(co, o))
                    # dindices.append(d)
                    # print (i,d)
                    if d>0.5:# and lcounter < 200:
                        
                        dindices.append(d)
                        # co = np.append(co, o, axis=0)

                        lcounter += 1
                    #     collect_paths.append(rest[i])
                dindices = np.array(dindices)
                if len(dindices)<20:
                    li = len(dindices)
                else:
                    li = 20
                # print (len(dindices), li)
                ind = np.argpartition(dindices, -li)[-li:]
                # print (ind)
                for i in range(len(rest)):
                    if i in ind:
                        collect_paths.append(rest[i])
