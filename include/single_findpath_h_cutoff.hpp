    ...
    -> std::vector<sorted_path>
{
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        if (num_next > current_search_width) {
            // if (next[current_search_width].s != nullptr) {

            // for (u = 0; u < num_next; u++) {
            //     fmt::print("sw: {}, d: {}: u: {} / {}, saddle_en: {}\n", current_search_width, d,
            //     u, num_next, next[u].saddle_en);

            // }

            int first_en = 0;
            first_en     = next[current_search_width].saddle_en;
            ...
            for (u = 0; u < num_next; u++) {
                if (next[u].s == nullptr) { break; }
                if (next[u].saddle_en == first_en and first_index == -1) { first_index = u; }
                if (next[u].saddle_en == first_en) { amount++; };
            }

            fmt::print(
                "sw: {}, d: {}, index: {}, first_en: {}, en[0]: {}, en[sw]: {}, amount: {}\n",
                current_search_width, d, first_index, first_en, next[0].saddle_en,
                next[current_search_width].saddle_en, amount);

            if (next[0].saddle_en == next[current_search_width].saddle_en){

                for (u = 1; u < num_next; u++) {

                    if (next[u].saddle_en == next[0].saddle_en) {
                        fmt::print("u: {} / {}, saddle_en: {}\n", u, num_next, next[u].saddle_en);
                    }
