...
class single_findpath
{
   ...
   public:
    ...
    single_findpath(std::string sequence, std::string s1, std::string s2, int search_width = 0, float search_width_multiplier=0,
    ...
    -> std::vector<sorted_path>
{
    ...
    // move_list.push_back(0);
    // sort move list so that we can "cluster" our moves
    std::sort(move_list.begin(), move_list.end(),
              [](const auto& a, const auto& b) -> bool { return a.i < b.i; });

    move_list.push_back({0, 0}); // last pos?? legacy?


    short lasti = move_list[0].i;
    short lastj = move_list[0].j;





    short lastclusterid = 1;

    std::vector<short> move_clusters{};
    ...
    for (auto& m : move_list){
        if (m.i == lasti) {
            ...
            // std::cout << m.i << "/" << m.j << "/" << lastclusterid << "\n";
            move_clusters.push_back(lastclusterid);
            continue; // start at pos 1

        }

        // if ((m.i == lasti -1 or m.i == lasti +1) and (m.j == lastj -1 or m.j == lastj +1)){
        //     // std::cout << "cluster" << "\n";
        //     int pass = 0;
        // }
        // else
        ...
        if ((m.i != lasti -1 and m.i != lasti +1) or (m.j != lastj -1 and m.j != lastj +1)){
            lastclusterid++;
        }
        // std::cout << m.i << "/" << m.j << "/" << lastclusterid << "\n";
    ...
    // if (move_list.size() == 1)
    //     move_clusters.push_back(-1);

    int cluster_total = lastclusterid;
    lastclusterid = move_clusters[0];

    // for (auto& m : move_clusters){
    //     std::cout << "cluster " << m << "/" << cluster_total <<"\n";
    // }

    std::vector<short> clusters_en(cluster_total, 0);
    std::fill(clusters_en.begin(), clusters_en.end(), 0);

    // for (auto& m : clusters_en){
    //     std::cout << "clusteren " << m << "/" << cluster_total <<"\n";
    // }
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        for (c = 0; current[c].s != nullptr; c++) {
            ...
            for (int a = 0; a < move_list.size(); a++) {
                ...
                auto curr_cluster = move_clusters[a];

                // compare prev moves from the same cluster
                // int minpos = 9999;
                // int maxpos = -9999;

                // for (int te = 0; te < move_list.size(); te++) {                    
                //     if (move_clusters[te] != curr_cluster)
                //         continue;
                //     const int past_move_index = current[c].moves[te];     
                //     if (past_move_index == 0 and te > maxpos) {
                //         maxpos = te;
                //     } 
                //     if (past_move_index == 0 and te < minpos) {
                //         minpos = te;
                //     } 
                //     // std::cout << "cluster: " << curr_cluster << " / a: " << a << " / prev:" << past_move_index <<  "\n";
                // } 
                ...
                if (i<0) {
                    ...
                    int a2 = a-1;
                    while (true) {
                        if (a2 == -1 or move_clusters[a2] != curr_cluster)
                            break;
                        if (current[c].moves[a2] == 0)
                            minpos = a2;
                        a2--;
                    }
                    // go right
                ...
                //             found = true;
                //         }
                //         if (current[c].moves[a2] == 0 and not found) {
                //             lower = a2;
                //         }

                //         if (current[c].moves[a2] == 0 and found)
                //         {
                //             upper = a2;
                //             break;
                //         }
                //         a2++;
                //     }
                //     if (found and a != lower and a != upper) {
                //         // std::cout << "minpos: " << minpos << " currpos: " << a << " " << i << "/" << j << "\n";
                //         // std::cout << "maxpos: " << maxpos << " currpos: " << a << " " << i << "/" << j << "\n";
                ...
                // currpos has to be maxpos or minpos
                // if (i<0 and minpos != 9999 and maxpos != -9999)
                ...
                if (en <= maxE) {
                    ...
                    auto curr_cluster = move_clusters[a];
                    if (clusters_en[curr_cluster] == 0) {
                        auto next_s = (en > current[c].saddle_en) ? en : current[c].saddle_en;
                        clusters_en[curr_cluster] = en;
                    } else {

                        // std::cout << "a " << a << " / curr cluster " << curr_cluster << " / compare " << en << " vs " << clusters_en[curr_cluster] << "\n";

                        if (i > 0 and en > clusters_en[curr_cluster]) {
                            //  std::cout << "ignore \n";
                            continue;
                        }
                        if (i > 0 and en < clusters_en[curr_cluster]) {
                            ...
                            next[num_next-1].s_hash = current[c].s_hash + h_list[a];
                            next[num_next-1].move_delete = dest_1;
                            next[num_next-1].move_i      = source_1;
                            next[num_next-1].move_j      = source_2;
                            next[num_next-1].length = current[c].length;
                            next[num_next-1].saddle_en =
                                (en > current[c].saddle_en) ? en : current[c].saddle_en;
                            next[num_next-1].curr_en = en;
                            next[num_next-1].last_id = c;
                            next[num_next-1].move_id = a;
                            continue;
                        }
        ...
        if (d <= bp_dist and flag) {
            ...
            // c      = 1;  // processed candidates
            u = -1;  // verified candidates without duplicates
            c = 0;   // processed candidates
        ...
        for (u = 0; u < current_search_width and u < num_next; u++) {
            ...
            // current[last_id].moves == nullptr
            // py::print (last_id, "test:", next[u].moves==nullptr,
            // current[last_id].moves==nullptr);
        ...
        // py::print("u:", u, "cw:", current_search_width, "num_next:", num_next);
