...
auto merge_once(const s_graph G1, const s_graph G2, short* pt_1, int s1_en, short* pt_2, int s2_en,
                bool direction, int total_bp_dist, int max_en, int merge_search_width,
                vrna_fold_compound_t* fc)
{
    ...
    for (int d = 1; d <= total_bp_dist; d++) {
        ...
        for (const auto current_path : all_paths) {
            ...
            available_edges(G1_node, 1, next_paths, current_path, fc, max_en, direction,
                            pool_offset, total_bp_dist, move_storage);
