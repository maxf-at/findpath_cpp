    ...
    -> std::vector<sorted_path>
{
    ...
    for (i = 1; i <= len; i++) {
        if (pt[i] != pt2[i]) {
            if (i < pt[i]) {
                ...
                move_list.push_back({static_cast<short>(-i), static_cast<short>(-pt[i]), 0});
                bp_dist++;
            }
    ...
    // for (i = 1; i <= bp_dist; i++) {
    //     fmt::print("i:{}, {} {} {} / {} {} {}\n", i, mlist[i].i, mlist[i].j, mlist[i].when, move_list[i].i, move_list[i].j, move_list[i].when);
    // }
    ...
    current[0].saddle_en = current[0].curr_en = vrna_eval_structure_pt(vc, pt);
        ...
        for (c = 0; current[c].s != nullptr; c++) {
            ...
            for (mv = current[c].moves; mv->i != 0; mv++) {
                ...
                if (en <= maxE) {
                    ...
                    next[num_next].saddle_en     = (en > current[c].saddle_en) ? en : current[c].saddle_en;
