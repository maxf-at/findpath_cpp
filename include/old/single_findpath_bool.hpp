                                  ...
                                  int final_search_width, bool mp) -> std::vector<sorted_path>
{
    ...
    fmt::print("{}:[{}, {}, {}, {}, {}],\n", final_search_width, t_a / 1000000, t_b / 1000000,
               t_c / 1000000, t_d / 1000000, t_e / 1000000);
...
auto bool_to_ptable(std::vector<bool>& pt_bool)
{
    ...
    short* pt = (short*)vrna_alloc(sizeof(short) * (n + 1));
    // short* stack = (short*)vrna_alloc(sizeof(short) * (n + 1));
    ...
    // return vrna_ptable_from_string(s.c_str(), 200);
    // return 1; /* success */
}
                                       ...
                                       intermediate_t* next, int dist, int bp_dist) -> int
{
    ...
    // std::cout << "dist: " << dist << " / ";
    // print_ptable_bool(*c.pt_b);

    // std::cout << "dist: " << dist << " \\ " << vrna_db_from_ptable(c.pt_a) << "\n";

    // (*pt_b_0).resize(300);
    // std::vector<bool>* pt_b_a = new std::vector<bool>(402);
    // ptable_to_bool(c.pt_a, *pt_b_a);

    // len = c.pt_a[0];
    len = 200;

    short* temp_pt = (short*)vrna_alloc(sizeof(short) * (len + 1));
    temp_pt        = bool_to_ptable(*c.pt_b);

    // this does not work with LOOP_EN

    // loopidx = vrna_loopidx_from_ptable(&c.pt_a[0]);
    loopidx = vrna_loopidx_from_ptable(temp_pt);
    ...
    for (mv = c.moves; mv->i != 0; mv++) {
        ...
        if (j < 0) {
            ...
            if ((loopidx[i] == loopidx[j]) &&          /* i and j belong to same loop */
                (temp_pt[i] == 0) && (temp_pt[j] == 0) /* ... and are unpaired */
            ) {
                source_1 = i;
                dest_1   = j;
                source_2 = j;
        ...
        if (en <= maxE) {
            ...
            next[num_next].pt_b  = new std::vector<bool>(400);
            *next[num_next].pt_b = *c.pt_b;

            // memcpy_fast(next[num_next].pt_b, c.pt_b, sizeof(std::vector<bool>(400)));
            if (dest_1 == 0) {
                (*next[num_next].pt_b)[source_1 * 2]     = false;
                (*next[num_next].pt_b)[source_1 * 2 + 1] = false;
                (*next[num_next].pt_b)[source_2 * 2]     = false;
                (*next[num_next].pt_b)[source_2 * 2 + 1] = false;
            } else {
                (*next[num_next].pt_b)[source_1 * 2]     = true;
                (*next[num_next].pt_b)[source_1 * 2 + 1] = true;
                (*next[num_next].pt_b)[source_2 * 2]     = true;
                (*next[num_next].pt_b)[source_2 * 2 + 1] = false;
            }

            // std::cout << "dist:C" << dist << " " << next[num_next].pt_b << " / ";
            // print_ptable_bool(*next[num_next].pt_b);
            // std::cout << "dist:D" << dist << " " << c.pt_b << " / ";
            // print_ptable_bool(*c.pt_b);
                                            ...
                                            int final_search_width) -> std::vector<sorted_path>
{
    ...
    std::vector<bool>* pt_b_0 = new std::vector<bool>(400);
    ptable_to_bool(pt, *pt_b_0);

    current[0].pt_b = new std::vector<bool>(400);
    ptable_to_bool(pt, *current[0].pt_b);

    // short* test_pt = bool_to_ptable(*pt_b_0);
    // print_ptable_bool(*pt_b_0);
    std::cout << "test: " << vrna_db_from_ptable(current[0].pt_a) << "\n";
    // std::cout << "test: " << vrna_db_from_ptable(test_pt) << "\n";
    ...
    for (d = 1; d <= dist; d++) {
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            ...
            // if ((*next[u].pt_b) != (*next[c].pt_b)) {
            // if (memcmp(&next[u].pt_b[0], &next[c].pt_b[0], (*next[c].pt_b).size()) != 0) {
            if (memcmp((*next[u].pt_b)[0], (*next[c].pt_b)[0], (*next[c].pt_b).size()) != 0) {
...
inline auto single_findpath::compare_ptable(const void* A, const void* B) -> int
{
    ...
    // c = memcmp(a->pt, b->pt, a->pt[0] * sizeof(short));
    // c = memcmp(a->pt_b, b->pt_b, (*a->pt_b).size());
    // c = memcmp(a->pt_b, b->pt_b, 400 * sizeof(bool));
    c = a->pt_b == b->pt_b;
