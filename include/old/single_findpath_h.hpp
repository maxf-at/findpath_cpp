    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            ...
            if (next[u].s_hash != next[c].s_hash) {
                // if u and c are different structures
                u++;
                next[u] = next[c];
                ...
                if (d >= bp_dist && next[c].saddle_en <= maxE &&
                    ...
                    // (valid for 2 iterations) memcpy(next[u].moves, current[last_id].moves,
                    // sizeof(short) * (bp_dist + 1)); next[u].moves[move_id] = d;  // update
                    // current move id to current basepair distance

                    // short* temp_moves = next[c].moves;
                    short* temp_moves = current[last_id].moves;

                    // fmt::print("F, {}, {}, {}  \n", move_id, temp_moves[move_id], d);

                    if (temp_moves[move_id] != 0) { fmt::print("ERROR\n"); }
                    ...
                    for (int temp_d = 0; temp_d < bp_dist; temp_d++) {
                        int i, j;

                        int current_dist = temp_moves[temp_d] - 1;
                        ...
                        i = move_list[temp_d].i;
                        j = move_list[temp_d].j;
