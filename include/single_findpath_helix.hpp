    ...
    -> std::vector<sorted_path>
{
    ...
    for (auto& m : move_list){
        if (m.i == lasti) continue; // start at pos 1
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        for (c = 0; current[c].s != nullptr; c++) {
            ...
            std::vector<short> valid_moves{};

            for (int a = 0; a < move_list.size(); a++) { // find valid moves
                ...
                // check if this cluster already had a move
                const short curr_helix = move_clusters[a];
                ...
                // while (true) {
                //     if (a + iter == 0) break;                    
                //     if (move_clusters[a+iter] != curr_helix)
                //         break;
                //     if (current[c].moves[a+iter] > maxpos){
                //         maxpos = current[c].moves[a+iter];
                //         adjid1 = a+iter;
                //     }
                //     iter--;
                // }
                ...
                // while (true) {
                //     py::print (a, iter);
                //     if (a + iter == move_list.size()) break;                    
                //     if (move_clusters[a+iter] != curr_helix)
                //         break;
                //     if (current[c].moves[a+iter] > maxpos)
                //     {
                ...
                if (maxpos > 0){
                    // fmt::print ("move {}/{} / {} / a: {} adjid {}/{} maxpos {} helixid {} \n", i,j, lastclusterid, a, adjid1, adjid2, maxpos, curr_helix);
   
                    // a has to be adjid +- 1
                    if (a - adjid1 != 1 and a - adjid1 != -1){
                        fmt::print ("err\n");
                        // continue;
                ...
                if (en <= maxE) {
                    ...
                    // if (next[num_next-1].move_i != 0 and lastclusterid>0 and lastclusterid == move_clusters[a]) {
                    if (num_next != 0 and lastclusterid>0 and lastclusterid == move_clusters[a]) {
                        
                        // next[num_next-1].saddle_en =
                        // (en > current[c].saddle_en) ? en : current[c].saddle_en;
                        const auto temps = (en > current[c].saddle_en) ? en : current[c].saddle_en;
                        // greedy helix addition
                        // fmt::print ("compare {} old: {} new: {}\n", lastclusterid, next[num_next-1].saddle_en, temps);
                        if (next[num_next-1].saddle_en > temps)
                        {
                            // fmt::print ("better\n");
                            ...
                            next[num_next-1].saddle_en = temps;
                        ...
                        {
                            // fmt::print ("worse {} {} vs {} {} / {}\n", i, j, next[num_next-1].move_i, next[num_next-1].move_j, num_next);
                            continue;
                        }
        ...
        // py::print ("py", d, bp_dist, num_next); // py::print == EVIL!
