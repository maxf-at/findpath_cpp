...
void test_quad(int* z_array, int* r_array, int max, const char* desc)
{
    ...
    if (max <= 10)
        printf("\e[1;31m%10d %10d %10d %10d %10d %10d %10d %10d %10d %10d\n", z_array[0],
               z_array[1], z_array[2], z_array[3], z_array[4], z_array[5], z_array[6], z_array[7],
               z_array[8], z_array[9]);
    ...
    if (max <= 10)
        printf("\e[1;32m%10d %10d %10d %10d %10d %10d %10d %10d %10d %10d\n", z_array[0],
               z_array[1], z_array[2], z_array[3], z_array[4], z_array[5], z_array[6], z_array[7],
               z_array[8], z_array[9]);

    printf("\e[0m        Quad Sort: sorted %d elements in %f seconds. (%s)\n", max,
           (end - start) / 1000000.0, desc);

    for (cnt = 1; cnt < max; cnt++)
        if (z_array[cnt - 1] > z_array[cnt]) {
            printf("        Quad Sort: not properly sorted at index %lld. (%d vs %d\n", cnt,
                   z_array[cnt - 1], z_array[cnt]);
            break;
        }
}
...
void test_quick(int* z_array, int* r_array, int max, const char* desc)
{
    ...
    for (cnt = 1; cnt < max; cnt++)
        if (z_array[cnt - 1] > z_array[cnt]) {
            printf("       Quick Sort: not properly sorted at index %lld. (%d vs %d\n", cnt,
                   z_array[cnt - 1], z_array[cnt]);
            break;
        }
}
...
int main(int argc, char** argv)
{
    ...
    for (cnt = 0; cnt < max; cnt++) { r_array[cnt] = cnt; }
    ...
    for (cnt = 0; cnt < max; cnt++) { r_array[cnt] = max - cnt; }
    ...
    for (cnt = 0; cnt < max * 3 / 4; cnt++) { r_array[cnt] = cnt; }

    for (cnt = max * 3 / 4; cnt < max; cnt++) { r_array[cnt] = rand(); }
