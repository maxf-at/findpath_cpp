...
static size_t hash_cstr(const char* s, int len)
{
    ...
    return std::hash<std::string_view>()(std::string_view(s, len));  // does not work
}
    ...
    -> std::vector<sorted_path>
{
    ...
    std::vector<std::vector<short>> move_storage(2 * bp_dist * current_search_width + 1);
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        for (c = 0; current[c].s != nullptr; c++) {
            ...
            for (int a = 0; a < move_list.size(); a++) {
                ...
                if (en <= maxE) {
                    ...
                    // next[num_next].s_hash = hash_c_string(next[num_next].s, len);

                    // next[num_next].s_hash = hash_c_string(next[num_next].s);
                    ...
                    // next[num_next].s_hash = hash_c_string(current[c].s, len);
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            ...
            if (memcmp(next[u].s, next[c].s, sizeof(char) * len) != 0) {
                // if u and c are different
                u++;
                next[u] = next[c];

                // next[num_next].last_id = c; // set last_id (from which node we came from)
                // next[num_next].move_id = a; // set move_id to current bp dist (further down after
                // sorting)
        ...
        for (u = 0; u < current_search_width and u < num_next; u++) {
            ...
            // current[last_id].moves[0]);

            memcpy(move_storage[u + pool_offset].data(), current[last_id].moves,
                   sizeof(short) * (bp_dist + 1));

            // fmt::print("done\n");

            next[u].moves          = move_storage[u + pool_offset].data();  // set move pointer
...
inline auto single_findpath::compare_ptable(const void* A, const void* B) -> int
{
    ...
    if (a->s_hash != b->s_hash) { 
        ...
        return a->s_hash < b->s_hash; // does not work...
