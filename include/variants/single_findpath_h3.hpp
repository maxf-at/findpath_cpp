    ...
    -> std::vector<sorted_path>
{
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        auto start = next;
        auto end   = std::min(current_search_width, num_next);
        ...
        if (d <= bp_dist and flag) {
            ...
            // py::print("dupes:", duplicates_found, "\tcurrent width", current_search_width, "\tprocessed:", u+1, "\tnum_next", num_next);
        ...
        for (u = 0; u < current_search_width and u < num_next; u++) {
            ...
            // py::print (last_id, "test:", next[u].moves==nullptr, current[last_id].moves==nullptr);
