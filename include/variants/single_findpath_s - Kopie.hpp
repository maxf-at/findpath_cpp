    ...
    -> std::vector<sorted_path>
{
    ...
    short* temp_pt   = vrna_ptable_copy(pt1);  // this just allocates the array
    ...
    for (i = 1; i <= len; i++) {
        if (pt1[i] != pt2[i]) {
            if (i < pt1[i]) {
                ...
                move_list.push_back({static_cast<short>(-i), static_cast<short>(-pt1[i]), 0});
                all_moves.push_back({static_cast<short>(-i), static_cast<short>(-pt1[i])});
                bp_dist++;
            }

            if (i < pt2[i]) {
                /* need to insert this pair */
                move_list.push_back({static_cast<short>(i), static_cast<short>(pt2[i]), 0});
                all_moves.push_back({static_cast<short>(i), static_cast<short>(pt2[i])});
                bp_dist++;
            }
        }
    }
    move_list.push_back({0, 0, 0});
    all_moves.push_back({0, 0});
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        // find valid moves (extracted original try moves function)
        for (c = 0; current[c].s != nullptr; c++) {
            int     en;
            move_t* mv;

            // update pairing table & loop table from current structure string
            ptable_from_string(temp_pt, temp_loop, current[c].s, stack);
            // int* loopidx = vrna_loopidx_from_ptable(temp_pt);

            for (int a=0; a<=all_moves.size(); a++) {

            // for (mv = current[c].moves; mv->i != 0; mv++) {

                int i = all_moves[a].i;
                int j = all_moves[a].j;
                int when = current[c].moves[a].when;
                ...
                if (en <= maxE) {
                    ...
                    next[num_next].s = s_pool[num_next + pool_offset].data(); // set structure pointer
                    ...
                    // memcpy(next[num_next].moves, current[c].moves, sizeof(move_t) * (bp_dist +
                    // 1));

                    memcpy(move_pool[num_next + pool_offset].data(), current[c].moves,
                           sizeof(move_t) * (bp_dist + 1));
                    next[num_next].moves = move_pool[num_next + pool_offset].data(); // set move pointer

                    // move_pool[num_next + pool_offset][1].when = bp_dist;
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            ...
            if (memcmp(next[u].s, next[c].s, sizeof(char) * len) != 0) {
                ...
                if (d >= bp_dist && next[c].saddle_en <= maxE &&
                    ...
                    // allocate bp_dist elements in vector
    ...
    if (current[0].moves) {
        // allocate bp_dist elements in vector
        sorted_path current_path(bp_dist);
        // auto& current_path = all_paths.emplace_back(bp_dist);

        std::qsort(current[0].moves, bp_dist, sizeof(move_t), compare_moves_when);
        for (d = 0; d < bp_dist; d++) {
            int i, j;
            i = current[0].moves[d].i;
            j = current[0].moves[d].j;
