                                       ...
                                       short* stack) -> int
{
    ...
    for (mv = c.moves; mv->i != 0; mv++) {
        ...
        if (en <= maxE) {
            ...
            // move_hash ^= 17 * source_1 * std::abs(source_2);
            // move_hash += (move_hash << 10);
            // move_hash ^= (move_hash >> 6);
            ...
            next[num_next].s_hash = c.s_hash + move_hash;



            // move_hash = std::hash<int>(source_1);
            // std::hash<std::string_view>()(std::string_view(s, len));


            // next[num_next].s_hash = hash_cstr(next[num_next].s, c.length - 1);
    ...
    -> std::vector<sorted_path>
{
    ...
    current[0].s_hash = 42; // "random" initial hash for the first structure
