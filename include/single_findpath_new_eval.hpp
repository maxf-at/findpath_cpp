...
class single_findpath
{
   ...
   public:
    ...
    single_findpath(std::string sequence, std::string s1, std::string s2, int search_width = 0,
                    float search_width_multiplier = 0, bool mp = true, int en_limit = INT_MAX - 1,
                    const py::dict& model_details = {})
            : sequence{sequence}, s1{s1}, s2{s2}
    {
        vrna_md_t md;
        ...
        if (search_width_multiplier > 0 or search_width == 0) {
            if (search_width_multiplier == 0) { search_width_multiplier = 2; }
...
int new_eval_move_pt(vrna_fold_compound_t* vc, short* pt, int m1, int m2, short * loop_energies,
                                                          int& k_result, int& k_loop_eval, int& i_result,
                                                           int& i_loop_eval)
{
    ...
    en_pre  = loop_energies[i];
    en_post = 0;
    if (m1 < 0) {
        /*it's a delete move */
        // en_pre += vrna_eval_loop_pt(vc, k, (const short*)pt);
        en_pre += loop_energies[k];
        pt[k] = 0;
        pt[l] = 0;
        k_result = k;
        k_loop_eval = 0;
        ...
        pt[l] = k;
        k_result = k;
        k_loop_eval = vrna_eval_loop_pt(vc, k, (const short*)pt);
        en_post += k_loop_eval;
    }

    i_result = i;
    i_loop_eval = vrna_eval_loop_pt(vc, i, (const short*)pt);
    en_post += i_loop_eval;
    /*  restore pair table */
    if (m1 < 0) {
        pt[k] = l;
    ...
    -> std::vector<sorted_path>
{
    ...
    // int ltemp_all = 0;
    short* loop_energies = vrna_ptable_copy(pt1);

    for (i = 0; i <= len; i++) {
        // int ltemp;
        if (temp_pt[i] == 0 or temp_pt[i] < i) {
            loop_energies[i] = 0;
        } else {
            loop_energies[i] = vrna_eval_loop_pt(vc, i, temp_pt);
        }
        // ltemp_all += ltemp;
        // int all_en = vrna_eval_structure_pt(vc, pt1);
        // fmt::print("i: {}, loop: {} / all: {} / pt[i]: {} / {}\n", i, ltemp, all_en, temp_pt[i],
        //    ltemp_all);
    }

    // fmt::print("{} {}\n", ltemp_all, vrna_eval_structure_pt(vc, pt1));
    ...
    std::vector<std::vector<short>> loop_energies_storage(2 * current_search_width + 4);
    for (auto& col : loop_energies_storage) { col.reserve(len + 1); }
    ...
    current[0].loop_energies = loop_energies;
    ...
    for (d = 1; d <= bp_dist; d++) {
        ...
        for (c = 0; current[c].s != nullptr; c++) {
            ...
            for (int a = 0; a < move_list.size(); a++) {
                ...
                int i_result;
                int i_loop_eval;
                en = current[c].curr_en + new_eval_move_pt(vc, temp_pt, i, j, current[c].loop_energies,
                                                           k_result, k_loop_eval, i_result,
                                                           i_loop_eval);

                // fmt::print ("{} / {} {} {} {}\n", en, k_result, k_loop_eval, i_result, i_loop_eval);
                ...
                if (en <= maxE) {
                    ...
                    next[num_next].k_result = k_result;
                    next[num_next].k_loop_eval = k_loop_eval;
                    next[num_next].i_result = i_result;
                    next[num_next].i_loop_eval = i_loop_eval;
        ...
        for (u = 0; u < current_search_width and u < num_next; u++) {
            ...
            next[u].loop_energies = loop_energies_storage[u + pool_offset].data();  // set structure pointer
            memcpy(next[u].loop_energies, current[last_id].loop_energies,
                   (current[last_id].length+1) * sizeof(short));

            int k_result = next[u].k_result;
            int k_loop_eval = next[u].k_loop_eval;
            int i_result = next[u].i_result;
            int i_loop_eval = next[u].i_loop_eval;
            next[u].loop_energies[k_result] = k_loop_eval;
            next[u].loop_energies[i_result] = i_loop_eval;
