...
class single_findpath
{
   private:
    ...
    static auto try_moves(vrna_fold_compound_t* vc, intermediate_t c, int maxE,
                          intermediate_t* next, int dist, int bp_dist, char* temp_s, short* temp_pt,
                          short* stack) -> int;
...
auto ptable_from_string(short* pt, char* s, short* stack)
{
    ...
    // short*       stack1;
    // stack1 = (short*)vrna_alloc(sizeof(short) * (n + 1));
...
inline auto single_findpath::try_moves(vrna_fold_compound_t* vc, intermediate_t c, int maxE,
                                       intermediate_t* next, int dist, int bp_dist, char* temp_s,
                                       short* temp_pt, short* stack) -> int
{
    ...
    for (mv = c.moves; mv->i != 0; mv++) {
        ...
        if (en <= maxE) {
            ...
            next[num_next].s = (char*)vrna_alloc(sizeof(char) * (len));
            memcpy(next[num_next].s, c.s, (len) * sizeof(char));
                                            ...
                                            int final_search_width) -> std::vector<sorted_path>
{
    ...
    char*  temp_s  = vrna_db_from_ptable(pt2);
    short* temp_pt = vrna_ptable_copy(pt1);
    short* stack   = vrna_ptable_copy(pt1);
    // short* stack = new short[200];
    // fmt::print("input: {}\n", vrna_db_from_ptable(pt2));
    // fmt::print("input: {}\n", vrna_db_from_ptable(pt1));
    ptable_from_string(temp_pt, temp_s, stack);
    ...
    for (d = 1; d <= dist; d++) {
        ...
        for (c = 0; current[c].pt != NULL; c++)
            ...
            num_next += try_moves(vc, current[c], maxE, next + num_next, d, bp_dist, temp_s,
                                  temp_pt, stack);
        if (num_next == 0) {
