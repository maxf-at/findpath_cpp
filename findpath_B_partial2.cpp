...
typedef struct intermediate {
    ...
    std::array<short, 301> current_pt;
    std::array<move_t, 95> current_moves;
...
// ub = upper bound?
{
    ...
    do {
        ...
        // std::future<std::vector<sorted_path>> ret2 = std::async(std::launch::async,
        // &find_path_once, vc, pt1, pt2, current_search_width, maxE, not direction, width);
                      ...
                      int dist, int bp_dist)
{
    ...
    len     = c.current_pt[0];
    loopidx = vrna_loopidx_from_ptable(c.current_pt.begin());
    oldE    = c.Sen;
    // for (mv = c.moves; mv->i != 0; mv++) {
    for (int k = 0; k < bp_dist; k++) {
        mv = c.moves + k;
        ...
        if (j < 0) {
            ...
            /* insert move */
            if ((loopidx[i] == loopidx[j]) &&                    /* i and j belong to same loop */
                (c.current_pt[i] == 0) && (c.current_pt[j] == 0) /* ... and are unpaired */
            ) {
                source_1 = i;
                dest_1   = j;
                source_2 = j;
...
#ifdef LOOP_EN
        en = c.curr_en + vrna_eval_move_pt(vc, c.current_pt.begin(), i, j);
#else
        en = vrna_eval_structure_pt(vc, pt);
#endif
        // this used to be en < maxE
        if (en <= maxE) {
            // next[num_next].current_pt
            std::copy(c.current_pt.begin(), c.current_pt.begin() + c.current_pt[0] + 1,
                      next[num_next].current_pt.begin());
            next[num_next].current_pt[source_1] = dest_1;
            next[num_next].current_pt[source_2] = dest_2;
            ...
            std::copy(c.current_moves.begin(), c.current_moves.begin()+bp_dist,
                      next[num_next].current_moves.begin());
                    ...
                    int maxE, bool direction, int final_search_width) -> std::vector<sorted_path>
{
    ...
    std::copy(pt1, pt1 + pt1[0] + 1, current_pt.begin());

    // printf("sample output: %s \n", vrna_db_from_ptable(pt));
    // printf("sample output: %s \n", vrna_db_from_ptable(current_pt.begin()));

    mlist = (move_t*)vrna_alloc(sizeof(move_t) * len); /* bp_dist < n */
    std::array<move_t, 95> current_move_list;
    ...
    for (i = 1; i <= len; i++) {
        if (pt[i] != pt2[i]) {
            if (i < pt[i]) {
                ...
                current_move_list[dist].i    = -i;
                current_move_list[dist].j    = -pt[i];
                current_move_list[dist].when = 0;
            ...
            if (i < pt2[i]) {
                ...
                current_move_list[dist].i    = i;
                current_move_list[dist].j    = pt2[i];
                current_move_list[dist].when = 0;
    ...
    // for (const auto& move : current_move_list) {
    //     IC(move.i, move.j, move.when, move.E);
    // }
    ...
    for (d = 1; d <= dist; d++) {
        ...
        {
            // IC(current[c].pt[0], current[c].current_pt[0], num_next);
            // printf("sample output: %s \n",
            // vrna_db_from_ptable(next[num_next].current_pt.begin()));
            num_next += try_moves(vc, current[c], maxE, next + num_next, d, bp_dist);
        }

        // IC(d, members, num_next);

        if (num_next == 0) {
            // case where we don't find any moves -> abort
            // for (cc = current; cc->pt != NULL; cc++)
            for (cc = current; members != 0; cc++) {
        ...
        for (u = 0, c = 1; c < num_next; c++) {
            if (memcmp(next[u].current_pt.begin(), next[c].current_pt.begin(),
                       sizeof(short) * len) != 0) {
                next[++u] = next[c];
    ...
    // path   = current[0].current_moves.begin();
...
PRIVATE void free_intermediate(intermediate_t* i)
{
    ...
    memset(i->current_pt.begin(), 0, 301);
    memset(i->current_moves.begin(), 0, 95);

    // std::fill(i->current_pt, i->current_pt+301, 0);
...
PRIVATE int compare_ptable(const void* A, const void* B)
{
    ...
    c = memcmp(a->current_pt.begin(), b->current_pt.begin(),
               a->current_pt.begin()[0] * sizeof(short));
    if (c != 0) return c;
