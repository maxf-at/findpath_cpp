...
auto ptable_hasher(const short* p_table) -> int
{
    // std::size_t h = 0;
...
std::unordered_map<std::string, int> ptable_map;
                      ...
                      int dist, int bp_dist)
{
    ...
    std::string temp = vrna_db_from_ptable(c.pt);
    ptable_map[temp]++;
                    ...
                    int maxE, bool direction, int final_search_width) -> std::vector<sorted_path>
{
    ...
    for (d = 1; d <= dist; d++) {
        ...
        //     std::string ts = vrna_db_from_ptable(next[k].pt);
        //     IC(ts, next[k].curr_en, next[k].Sen);
        // }
...
PRIVATE int compare_ptable(const void* A, const void* B)
{
    ...
    if (c != 0) {
        // std::string st1 = vrna_db_from_ptable(a->pt);
        // std::string st2 = vrna_db_from_ptable(b->pt);
